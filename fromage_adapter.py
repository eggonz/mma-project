...
class FromageModel:
  ...
  def prompt(self, image: PIL.Image, text:str, nr_of_retrieval: int = 1, debug = False) -> List[Union[PIL.Image, int]]:
    '''
    ...
    output = self.model.generate_for_images_and_texts(prompt = prompt,
                                            max_img_per_ret=nr_of_retrieval,
                                            num_words=0)
    if debug:
      outputs = list(map(lambda x: x[0], output))
